...
def generate_plots(
    ...
    *,
    ...
    suffix = "verify_as_negative" if include_verify_as_negative else "default"
    paths = [
    ...
    # Also write canonical filenames for default mode (supervisor workflow)
    if not include_verify_as_negative:
        canonical = plots_dir / "confusion_matrix.png"
        if paths[0].exists():
            canonical.write_bytes(paths[0].read_bytes())
